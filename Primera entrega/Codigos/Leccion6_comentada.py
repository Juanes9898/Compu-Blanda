...
# --------------------------------------------------------------------
        ...
        # se realiza un ciclo que identifica cada linea, si la hay, poniendo un estilo y color diferente
    ...
    # se hace que la figura quepa dentro de un espacio, sin importar el tamaño
    # se le dice que el limite donde parte el vector Y es 0
    # se hacen dos ciclos que verifican los limites de x y Y
    # se le dice al programa que dibuje, que grafique
    # siempre y cuando se cumpla la condicion de los limites establecidos, con el estilo de linea, y color alli dispuestos
    # se guarda la figura
...
# -----------------------------------------------------------------
# usando la funcion polyfit, que toma los puntos x y los y,
#  y el numero 1 indica que busca una recta que se aproxime a esos puntos
# el numero 2, se refiere a que combina rect y parabola que se aproximen a los puntos
# full=True, todo lo que generó el ajuste lo devuelve
# como devuelve varios datos en paralelo tomaremos por utilidad fp1 y fp2, y res1 y res 2
# fp1 y fp2 son los valores que necesita toda recta x y y
# residuals es el error que se calculo para la recta
# se imrpime fp1 y fp2
# se imprime el error res1 y res2 (el menor numero de diferencia entre las rectas)
# toma los valores de fp1 y fp2 y los guarda en f1 y f2 para crear una lista
...
# se dibujan las rectas para las funciones en orden 3, 10 y 100
...
# de inflexión
# -----------------------------------------------------------------
# se calcula el punto de inflexion, por el numero de horas de la semana (despues de 3 semanas y media)
# inflexion toma un valor que va a ser usado para calcular ahora la grafica tomando los puntos
# x y y antes y despues del punto de inflexion
inflexion = 3.5 * 7 * 24
xa = x[:int(inflexion)]
ya = y[:int(inflexion)]
xb = x[int(inflexion):]
...
# -----------------------------------------------------------------
# se grafican las rectas con el valor que multiplica a la x y el desplazamiento en y
# la funcion es creada mediante la funcion polyfit que toma los valores de x y y, y dibuja una recta aporximada
...
# recibe una funcion, y todos los puntos x, los puntos y
# suma los valores simulados en x menos los valores reales y y los eleva al cuadrado
...
# -----------------------------------------------------------------
# Se imprimen los errores para cada funcion de orden 1, 2, 3, 10, y 100
# tambien se imprime de igual manera la recta del error desde el puinto de inflexion (3 semanas y media)
...
# -----------------------------------------------------------------
# Se crea un modelo de aproximación
# se le envian los vectores X y Y, se le envian el polinomio en grado 1, 2, 3, 10, 100
# lo que se genere, lo graba en el archivo .png, en la carpeta CHART_DIR
# se contruye el eje X, usando los valores indicados
# parte desde 0 y llega hasta el valor de 10000
plot_models(
